...
const notificationDeleteButtons = document.querySelectorAll('.notification-delete-button');
...
function addEventListenersToWebSocket(socket) {
    ...
    socket.onmessage = (event) => {
        ...
        let newToast = new bootstrap.Toast(newToastElement, { delay: toastDisplayDuration });
        newToast.show();
...
function addEventListenersToNotificationElement(element) {
    ...
    element.addEventListener('click', () => {
        let notificationId = element.getAttribute('id').replace('notification-', '');
        let xmlHttp = new XMLHttpRequest();
        xmlHttp.open('POST', `${location.origin}/notification/${notificationId}/read/`, false);
        xmlHttp.setRequestHeader('X-CSRFToken', getCookie('csrftoken')), xmlHttp.send(null);
...
function addEventListenerToNotificaionDeleteButton(button) {
    button.addEventListener('click', (event) => {
        ...
        notificationElement.style.maxHeight = notificationElement.offsetHeight + 'px';
...
function createNotificationElement(id, title, message, link, timestamp) {
    ...
    notificationTime.innerText = luxon.DateTime.fromJSDate(new Date(timestamp)).toFormat(
        'HH:mm dd-MM-yyyy'
    );
    ...
    deleteButton.classList.add('notification-delete-button', 'btn', 'btn-danger', 'p-1');
...
function createToastElement(id, title, message, time, link = null) {
    ...
    headerTime.innerText = luxon.DateTime.fromJSDate(new Date(time)).toFormat('HH:mm dd-MM-yyyy');
    ...
    if (link) {
        toast.addEventListener('click', () => {
            let xmlHttp = new XMLHttpRequest();
            xmlHttp.open('POST', `${location.origin}/notification/${id}/read/`, false);
            xmlHttp.setRequestHeader('X-CSRFToken', getCookie('csrftoken')), xmlHttp.send(null);
