...
class TestSyncUnreadNotificationCounts(TestCase):
    def setUp(self) -> None:
        self.u1 = User.objects.create_user(
            username="u1", email="u1@test.com", unread_notifications_count=0
        )
        self.u2 = User.objects.create_user(
            username="u2", email="u2@test.com", unread_notifications_count=99
        )
        self.u3 = User.objects.create_user(
            username="u3", email="u3@test.com", unread_notifications_count=1
        )
        self.u4 = User.objects.create_user(
            username="u4", email="u4@test.com", unread_notifications_count=5
        )
    ...
    def test_syncs_drifted_counts_and_returns_stats(self) -> None:
        ...
        self.assertEqual(
            stats,
            {"users_checked": 4, "users_updated": 4, "users_zeroed": 2},
        )
    ...
    def test_sync_updates_only_selected_users_when_user_ids_are_provided(self) -> None:
        ...
        stats = sync_unread_notification_counts(
            user_ids=[self.u1.id, self.u2.id], batch_size=10
        )

        self.assertEqual(
            stats,
            {"users_checked": 2, "users_updated": 2, "users_zeroed": 0},
        )
    ...
    def test_sync_with_no_drift_returns_zero_updates(self) -> None:
        ...
        self.assertEqual(
            stats,
            {"users_checked": 4, "users_updated": 0, "users_zeroed": 0},
        )
    ...
    def test_sync_with_empty_user_ids_checks_no_users(self) -> None:
        stats = sync_unread_notification_counts(user_ids=[], batch_size=10)
    ...
    def test_sync_accepts_generator_for_user_ids(self) -> None:
        ...
        stats = sync_unread_notification_counts(
            user_ids=user_ids_generator, batch_size=10
        )
...
class TestCounterHelpers(TestCase):
    def setUp(self) -> None:
        self.u1 = User.objects.create_user(
            username="u1", email="u1@test.com", unread_notifications_count=7
        )
        self.u2 = User.objects.create_user(
            username="u2", email="u2@test.com", unread_notifications_count=0
        )
        self.u3 = User.objects.create_user(
            username="u3", email="u3@test.com", unread_notifications_count=2
        )
    ...
    def test_plan_unread_count_updates_splits_zero_and_nonzero_updates(self) -> None:
        ...
        actual_unread_counts = {self.u1.id: 0, self.u2.id: 0, self.u3.id: 5}
    ...
    def test_apply_unread_count_updates_updates_rows_and_returns_stats(self) -> None:
        updated, zeroed = _apply_unread_count_updates(
            ids_to_zero=[self.u1.id], ids_to_fix_by_count={5: [self.u3.id]}
        )
    ...
    def test_plan_unread_count_updates_groups_changed_counts(self) -> None:
        user_batch = [
            {"id": 1, "unread_notifications_count": 0},
            {"id": 2, "unread_notifications_count": 3},
        ]
        actual_unread_counts = {1: 0, 2: 1}
